...
namespace MazeParty.Multiplayer
{
    ...
    public sealed class NetworkMatchState : NetworkBehaviour
    {
            ...
            out uint inputEpoch)
        {
            ...
            return runtime.TryGetRoundAndInputEpoch(out roundNumber, out inputEpoch);
        }
            ...
            uint inputEpoch = 0U)
        {
            if (TryGetCurrentMinigameRuntime(out var runtime))
            {
                runtime.ReceiveMovementInputOnServer(
                    avatar,
        ...
        public void RoutePushInputOnCurrentMinigameOnServer(NetworkPlayerAvatar avatar)
        {
            if (TryGetCurrentMinigameRuntime(out var runtime))
            {
                runtime.RequestPushOnServer(avatar);
            }
        }
            ...
            uint inputEpoch)
        {
            if (TryGetCurrentMinigameRuntime(out var runtime))
            {
                runtime.SetInflateHeldOnServer(
                    avatar,
                    isHeld,
                    roundNumber,
                    inputEpoch);
            ...
            uint inputEpoch)
        {
            if (TryGetCurrentMinigameRuntime(out var runtime))
            {
                runtime.RequestPrimaryActionOnServer(
                    avatar,
                    roundNumber,
                    inputEpoch);
            }
            ...
            Vector2 input)
        {
            if (TryGetCurrentMinigameRuntime(out var runtime))
            {
                runtime.TrySonarOnServer(avatar, Vector2.ClampMagnitude(input, 1f));
            }
        }
            ...
            WrongWayDirection direction)
        {
            if (TryGetCurrentMinigameRuntime(out var runtime))
            {
                runtime.TrySubmitDirectionOnServer(avatar, direction);
            }
        }
        ...
        public bool IsSnowySpinPlaying =>
            IsSnowySpinPhase &&
            FlowState == BoardFlowState.MinigamePlaying;
        public bool IsArenaCombatPhase =>
            GameplayEnabled &&
        ...
        public bool IsArenaCombatPlaying =>
            IsArenaCombatPhase &&
            FlowState == BoardFlowState.MinigamePlaying;
        public bool IsCliffBarragePhase =>
            GameplayEnabled &&
        ...
        {
            if (avatar == null || !avatar.IsSpawned ||
                IsGlobalSimulationPaused || !avatar.IsCombatAlive)
            {
                return false;
            }

            if (IsArenaCombatPlaying)
            {
                return NetworkArenaCombatState.Instance != null &&
                       NetworkArenaCombatState.Instance.CanAcceptInputForSlot(
                           avatar.AssignedSlot);
            }

            return IsCombatPhase && IsCombatActive &&
                   IsCombatParticipant(avatar.AssignedSlot) &&
                   IsCombatAlive(avatar.AssignedSlot);
        }
