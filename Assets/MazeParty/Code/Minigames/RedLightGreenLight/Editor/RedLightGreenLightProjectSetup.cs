...
namespace MazeParty.Editor
{
    ...
    public static class RedLightGreenLightProjectSetup
    {
        ...
        private const string ScenesFolder = "Assets/MazeParty/Scenes/Minigames/RedLightGreenLight";
        private const string UiFolder = "Assets/MazeParty/Prefabs/Minigames/RedLightGreenLight";
        private const string UiPrefabFolder = "Assets/MazeParty/Prefabs/Minigames/RedLightGreenLight/UI";
            ...
            out Light redLight)
        {
            ...
            var connectedTower = MinigameCorePrefabUtility.Connect(
                tower,
                SignalTowerPrefabPath);
            var connectedGreen = connectedTower.transform.Find(
                "Green Signal");
            var connectedRed = connectedTower.transform.Find(
                "Red Signal");
            greenRenderer = connectedGreen != null
                ? connectedGreen.GetComponent<Renderer>()
                : null;
            redRenderer = connectedRed != null
                ? connectedRed.GetComponent<Renderer>()
                : null;
            greenLight = connectedGreen != null
                ? connectedGreen.Find("Green Signal Light")?.GetComponent<Light>()
                : null;
            redLight = connectedRed != null
                ? connectedRed.Find("Red Signal Light")?.GetComponent<Light>()
                : null;
            if (greenRenderer == null || redRenderer == null ||
                greenLight == null || redLight == null)
            {
                throw new InvalidOperationException(
                    "SignalTower.prefab must retain both signal renderers " +
                    "and their named lights.");
            }
        }
