...
namespace MazeParty.Editor
{
    ...
    public static class StableFootingProjectSetup
    {
            ...
            StableFootingMaterials materials)
        {
            ...
            anchor = MinigameCorePrefabUtility.Connect(
                anchor.gameObject,
                CorePrefabFolder + "/Tile.prefab").transform;
            anchor.name = "Tile Anchor " + tileIndex.ToString("00");
            var connectedCross = anchor.Find("Cross Mark");
            var connectedCircle = anchor.Find("Circle Mark");
            var connectedSquare = anchor.Find("Square Mark");
            if (connectedCross == null || connectedCircle == null ||
                connectedSquare == null)
            {
                throw new InvalidOperationException(
                    "StableFooting Tile.prefab must retain all three " +
                    "symbol roots for runtime selection.");
            }
            connectedCross.gameObject.SetActive(visibleSymbol == 0);
            connectedCircle.gameObject.SetActive(visibleSymbol == 1);
            connectedSquare.gameObject.SetActive(visibleSymbol == 2);
        }
            ...
            StableFootingMaterials materials)
        {
            ...
            display = MinigameCorePrefabUtility.Connect(
                display.gameObject,
                CorePrefabFolder + "/SafeSymbolDisplay.prefab").transform;
            var connectedCross = display.Find("Cross Mark");
            var connectedCircle = display.Find("Circle Mark");
            var connectedSquare = display.Find("Square Mark");
            if (connectedCross == null || connectedCircle == null ||
                connectedSquare == null)
            {
                throw new InvalidOperationException(
                    "StableFooting SafeSymbolDisplay.prefab must retain " +
                    "all three symbol roots.");
            }
            return new SafeDisplayReferences
            {
                CrossRenderer = connectedCross.GetComponentInChildren<Renderer>(true),
                CircleRenderer = connectedCircle.GetComponent<Renderer>(),
                SquareRenderer = connectedSquare.GetComponent<Renderer>()
            };
        }
