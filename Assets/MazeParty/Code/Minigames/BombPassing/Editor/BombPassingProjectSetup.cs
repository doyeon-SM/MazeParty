...
{
    /// <summary>
    /// Builds the shared-camera Bomb Passing world. The network view owns
    /// player visuals; the bomb is prefab-authored and its design survives a
    /// scene rebuild. Existing material assets are also reused.
    /// </summary>
    public static class BombPassingProjectSetup
    {
            ...
            Transform parent, Materials materials)
        {
            ...
            // Keep the fuse, body and warning light editable as one asset.
            bomb = MinigameCorePrefabUtility.Connect(bomb,
                CorePrefabFolder + "/Bomb.prefab");
            bombLight = bomb.GetComponentInChildren<Light>(true);
            if (bombLight == null || bomb.GetComponent<Renderer>() == null)
            {
                throw new InvalidOperationException(
