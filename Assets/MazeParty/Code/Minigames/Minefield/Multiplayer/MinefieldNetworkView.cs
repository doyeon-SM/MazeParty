...
namespace MazeParty.Multiplayer
{
    ...
    public sealed class MinefieldNetworkView : MonoBehaviour
    {
        ...
        {
            ResolveSceneReferences();
            if (sirenPrefab == null || sonarPulsePrefab == null ||
                mineMarkerPrefab == null)
            {
                if (!_corePrefabContractErrorLogged)
                {
                    Debug.LogError(
                        "MinefieldNetworkView requires Siren, Sonar Pulse " +
                        "and Detected Mine prefab bindings. Rebuild the " +
                        "Minefield scene in the Editor.",
                        this);
                    _corePrefabContractErrorLogged = true;
                }
                return false;
            }
            for (var slot = 0; slot < _runners.Length; slot++)
        ...
        private RunnerView CreateRunner(int slot)
        {
            ...
            var siren = Instantiate(
                ...
                false);
            siren.name = "Proximity Siren";
            siren.transform.localPosition = new Vector3(0f, 0.72f, 0f);
            var lensRenderer = FindDescendant(
                siren.transform,
                "Siren Red Lens")?.GetComponent<Renderer>();
            var warningLight = FindDescendant(
                siren.transform,
                "Siren Red Light")?.GetComponent<Light>();
            var pulseObject = Instantiate(
                sonarPulsePrefab,
                runnerObject.transform,
                false);
            pulseObject.name = "Sonar Pulse";
            pulseObject.transform.localPosition = new Vector3(0f, 0.06f, 0f);
            var pulse = pulseObject.GetComponent<LineRenderer>();
            if (lensRenderer == null || warningLight == null || pulse == null)
            {
                throw new InvalidOperationException(
                    "Minefield core prefabs are missing their lens, light " +
                    "or sonar LineRenderer contract.");
            }
            pulse.enabled = false;
        ...
        private void RebuildMineMarkers(IReadOnlyList<Vector3> positions)
        {
            ...
            {
                var marker = Instantiate(mineMarkerPrefab, mineRoot, false);
