...
7. 선택이 끝나면 주사위를 굴리기 전에도 WASD로 현재 칸 안을 이동할 수 있다.
   ...
   주사위를 조준한 상태에서 우클릭하면 서버 판정 1~12 굴림을 시작한다.
   다른 플레이어의 주사위는 밀거나 굴릴 수 없다.
8. 서버가 결과를 먼저 균등 추첨한 뒤 D12가 0.65초 동안 물리적으로 구르고,
   0.35초 동안 해당 숫자 면이 위로 오도록 착지한다. 총 1초의 착지가 끝날 때
   이동 수가 확정되며 결과는 주사위 윗면과 오브젝트 위 숫자로 모든 플레이어가
   직접 확인한다. 결과 확인 시간 1초가 지나면 월드 주사위는 사라진다. 열린 방향의
...
9. 전원이 목적 칸에 도착하거나 3분이 끝나면 상승 카메라 전환과 정리 시간을
   거친다. 공개된 미니게임 규칙을 확인하고 네 명 모두 `READY`를 누르면 동기화된
   additive 미니게임 씬으로 전환하며, 결과 정산 뒤 다음 턴으로 돌아온다.

`Gift Grab`은 서버 권한 공용 탑뷰 경기장에서 60초씩 2라운드 진행한다.
시작 선물 10개에 15·30·45초마다 3개가 추가되어 라운드당 총 19개가
등장한다. WASD로 이동하고, 빈손 좌클릭은 전방 밀기이며 선물을 든 좌클릭은
바라보는 방향으로 던지기다. 보관된 선물만 점수에 포함되고 상대 영역의
보관 선물은 접촉해 훔칠 수 있다. 라운드 점수 동률은 두 라운드에서 실제로
보관한 선물 총합, 마지막 라운드 순위, 서버 좌석 순으로 결정한다. 이동·충돌,
기절, 밀기 쿨다운, 투척 첫 충돌과 선물 소유권은 모두 서버가 판정한다.

게임 플레이 좌석은 NGO 서버가 연결된 플레이어 오브젝트에 0~3번을
직접 배정한다. 클라이언트가 좌석 값을 제출하거나 선점하지 않는다.
로비의 MPS 좌석은 준비/정원 검증용 메타데이터이므로 화면에는 번호로
표시하지 않는다.
주사위 값과 월드 물리는 서버에서 판정한다. 월드 주사위는 추적되는 D12 FBX와
512px 텍스처를 사용하며 플레이어별 빨강·파랑·초록·노랑 재질로 구분한다.
정확한 결과는 소유자 HUD와 월드 주사위 오브젝트에 1초 동안 표시되며,
다른 참가자의 HUD에는 숫자를 노출하지 않는다.
